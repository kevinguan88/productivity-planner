...
export default function HabitTracker() {
    ...
    useEffect(() => {   
        const loadHabits = async (showRefreshing = false) => {
            ...
            try {
                ...
                const habitsData = await HabitService.refreshHabitsWithCounts()
        ...
        const initialLoad = async () => {
            try {
                ...
                const cachedHabits = await HabitService.getHabitsWithCounts()
    ...
    const handleAddHabit = async (newHabit) => {
        try {
            const addedHabit = await HabitService.addHabit(newHabit.title, newHabit.color)
            if (addedHabit) {
                // Optimistic update - add to UI immediately
                const newHabitWithCount = {
                    ...addedHabit,
                    icon_name: newHabit.icon_name || '',
                    count: 0,
                ...
                // Then refresh in background to get accurate counts
                setTimeout(async () => {
                    const habitsData = await HabitService.refreshHabitsWithCounts()
                    setHabits(habitsData)
                }, 500)
